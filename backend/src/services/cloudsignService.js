...
const signerDocumentCloudSign = async (documentId) => {
  try {
    // Fichier fixe "conge.pdf"
    const fileName = 'conge.pdf';
    ...
    console.log(`✅ Document conge.pdf signé avec succès :`, response.data);
    return response.data;

  } catch (error) {
    if (error.response) {
      console.error(`❌ Erreur CloudSign conge.pdf :`, error.response.data);
    } else {
      console.error(`❌ Erreur CloudSign conge.pdf :`, error.message);
    }
    throw error;
  }
};
